...
def load_data_bundle(cfg: AppConfig) -> Dict[str, List[Dict]]:
    ...
    request_details = load_dataset(cfg.processed_dir, "matres_request_details.csv")
    ...
    return {
        ...
        "request_details": request_details.to_dict("records"),
    }
...
def build_layout(app: Dash, cfg: AppConfig) -> html.Div:
    ...
    request_details = pd.DataFrame(data_bundle.get("request_details", []))
...
def register_callbacks(app: Dash, cfg: AppConfig) -> None:
    ...
    def update_visuals(data, role_value):
        ...
        pde_alerts = pd.DataFrame(data.get("pde_alerts", []))
        request_details = pd.DataFrame(data.get("request_details", []))
    ...
    def update_drill_details(data, active_cell, summary_rows):
        details = pd.DataFrame(data.get("request_details", []))
