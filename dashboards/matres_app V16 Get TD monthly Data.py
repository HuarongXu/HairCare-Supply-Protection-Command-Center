...
def format_month_label(label: str) -> str:
    """Render YYYY-MM style labels into a friendlier year·month tag for charts."""
    try:
...
def format_month_label_slash(label: str) -> str:
    """Render YYYY-MM month labels into YYYY/MM for table headers."""
    try:
        period = pd.Period(label)
...
def build_hc_idp_monthly_table(df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
    if df.empty:
        return [{"name": "Prod Line AS", "id": "Prod Line AS"}, {"name": "Overall Result", "id": "Overall Result"}], []

    working = df.copy()
    if "Prod Line AS" not in working.columns:
        first_col = working.columns[0]
        working = working.rename(columns={first_col: "Prod Line AS"})

    value_cols = [col for col in working.columns if col != "Prod Line AS"]
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(
                        className="demand-table-card",
                        children=[
                            html.H3("Table 2: Supply Protection"),
                            dcc.Loading(
                                DataTable(
