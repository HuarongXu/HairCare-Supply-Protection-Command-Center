...
def build_production_data_table_by_plant(df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
    ...
    append_row("Total", grouped["Plant"].astype(str).str.strip().tolist())
...
def build_production_data_table_by_plant_level(df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
    ...
    month_cols = sorted([c for c in working.columns if re.fullmatch(r"\d{4}-\d{2}", str(c))])
    ordered_cols = ["Plant", "Level1", "Level2", *month_cols]

    for col in month_cols:
        working[col] = pd.to_numeric(working[col], errors="coerce").fillna(0.0)

    if month_cols:
        working = working[working[month_cols].abs().sum(axis=1) > 0].copy()

    working = working.sort_values(["Plant", "Level1", "Level2"], ascending=[True, True, True]).reset_index(drop=True)

    result_rows: List[Dict[str, Any]] = []

    def append_plant_detail(plant: str) -> None:
        subset = working[working["Plant"].astype(str).str.strip() == plant].copy()
        if subset.empty:
            return
        for _, row in subset.iterrows():
            ...
            for month in month_cols:
                record[month] = pd.to_numeric(row.get(month), errors="coerce")
            result_rows.append(record)
    ...
    def append_group_total(label: str, plants: List[str]) -> None:
        subset = working[working["Plant"].astype(str).str.strip().isin(plants)].copy()
        if subset.empty:
            month_sum = {month: 0.0 for month in month_cols}
        else:
            month_sum = {
                month: pd.to_numeric(subset[month], errors="coerce").fillna(0.0).sum(min_count=1)
                for month in month_cols
            }
        record = {
            "Plant": label,
    ...
    all_plants = working["Plant"].astype(str).str.strip().unique().tolist()
    append_group_total("Total", all_plants)

    result_df = pd.DataFrame(result_rows)
    for month in month_cols:
        result_df[month] = result_df[month].apply(lambda v: "-" if pd.isna(v) or float(v) == 0.0 else f"{float(v):,.0f}")
...
def register_callbacks(app: Dash, cfg: AppConfig) -> None:
    ...
    def update_visuals(data, role_value, drill_role_value):
        ...
        drill_columns, drill_rows = build_role_item_project_summary(request_details, selected_drill_role)
        hc_idp_columns, hc_idp_rows = build_hc_idp_monthly_table(hc_idp_monthly)
        hc_idp_hs_df = build_demand_hs_dataframe(hc_idp_monthly, monthly_level1)
        hc_idp_iya_columns, hc_idp_iya_rows = build_demand_iya_table(hc_idp_monthly, historical_shipment)
        hc_idp_hs_columns, hc_idp_hs_rows = build_hc_idp_monthly_table(hc_idp_hs_df)
